...
import { LayoutDashboard, FileText, Search, MessageSquare, Settings, LogOut, ShieldAlert, BarChart3, ListTodo, X } from 'lucide-react';
...
const Sidebar = ({ mobileOpen, onClose }) => {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="w-64 bg-slate-900 border-r border-slate-800 text-slate-300 flex-col h-full shrink-0 transition-all duration-300 shadow-xl hidden md:flex">
        {sidebarContent}
      </aside>
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside
              ...
              exit={{ x: -280 }}
              transition={{ type: 'spring', stiffness: 300, damping: 30 }}
              className="fixed top-0 left-0 w-64 h-full bg-slate-900 border-r border-slate-800 text-slate-300 flex flex-col z-50 shadow-2xl md:hidden"
            >
              {sidebarContent}
            </motion.aside>
          </>
